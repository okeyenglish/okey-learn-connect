-- Fix function to include ALL chats with unread messages regardless of date
...
BEGIN
  RETURN QUERY
  WITH 
  -- Get clients with unread messages (always include these)
  unread_clients AS (
    SELECT DISTINCT cm.client_id
    FROM chat_messages cm
    WHERE cm.is_read = false AND cm.message_type = 'client'
  ),
  -- Get recent clients by last message time
  recent_by_time AS (
    SELECT 
      cm.client_id,
  ...
  -- Combine: all unread + recent by time (union removes duplicates)
  combined_clients AS (
    SELECT client_id FROM unread_clients
    UNION
    SELECT client_id FROM recent_by_time
  ),
  ...
  recent_clients AS (
    ...
    FROM combined_clients cc
    JOIN clients c ON c.id = cc.client_id AND c.is_active = true
    JOIN chat_messages cm ON cm.client_id = cc.client_id
    ORDER BY cc.client_id, cm.created_at DESC
  ),
  unread_stats AS (
    SELECT 
      cm.client_id,
  ...
  ORDER BY 
    ...
    CASE WHEN COALESCE(us.unread_count, 0) > 0 THEN 0 ELSE 1 END,
    rc.last_message_time DESC;
END;
$$;