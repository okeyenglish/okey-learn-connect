-- Create index for sorting by date (critical for ORDER BY created_at DESC queries)
CREATE INDEX IF NOT EXISTS idx_chat_messages_created_desc 
ON chat_messages (created_at DESC);

-- Create optimized RPC function for fast chat threads loading
...
BEGIN
  ...
  unread_counts AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE cm.messenger_type = 'whatsapp' OR cm.messenger_type IS NULL) as unread_whatsapp,
      ...
      COUNT(*) FILTER (WHERE cm.messenger_type = 'calls') as unread_calls,
  ...
  SELECT 
    ...
    COALESCE(uc.unread_email, 0) as unread_email,
...
GRANT EXECUTE ON FUNCTION get_chat_threads_fast TO authenticated;
GRANT EXECUTE ON FUNCTION get_chat_threads_fast TO anon;