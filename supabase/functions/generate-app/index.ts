...
- ⚠️ ОБЯЗАТЕЛЬНО: Добавьте padding к body (минимум 20px сверху) чтобы контент не обрезался
- ⚠️ ОБЯЗАТЕЛЬНО: Контейнер приложения должен иметь max-width и margin: 0 auto для центрирования
- НЕ используйте position: fixed для основного контейнера без правильного позиционирования
...
`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        ...
        return new Response(
          JSON.stringify({ error: 'Недостаточно средств на балансе Lovable AI.' }),
