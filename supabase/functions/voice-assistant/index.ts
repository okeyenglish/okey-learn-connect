...
5. "удали задачу", "удали задачи", "убери задачу", "отмени задачу", "удали на завтра" = delete_task (НЕ get_tasks!)
...
✅ "Удали все задачи на завтра" 
   → delete_task: {dueDate: "2025-09-22"}

✅ "Удали задачу у клиента Даниил"
   → delete_task: {clientName: "Даниил"}

✅ "Удали задачу связаться с клиентом"
   → delete_task: {taskTitle: "связаться с клиентом"}
...
- Если команда просит удалить задачи ("удали задачу", "убери задачу", "отмени задачу", "удали все задачи", "удали на завтра") - ВСЕГДА используй delete_task!
- При удалении можно указывать: конкретный ID задачи, имя клиента, заголовок задачи или дату (завтра, на определенную дату)!
- Если команда содержит "завтра", "на завтра" при удалении - используй параметр dueDate с завтрашней датой!
...
Команда пользователя: "${userCommand}"`;
    ...
    const functions = [
      ...
      {
        name: "delete_task",
        description: "Удалить задачи (устанавливает статус cancelled). Можно удалять по ID, клиенту, заголовку или дате",
        parameters: {
          type: "object",
          properties: {
            ...
            dueDate: { type: "string", description: "Дата для удаления задач в формате YYYY-MM-DD (например, завтра)" }
          }
        }
      },
      {
            ...
            case 'delete_task':
              ...
              if (functionArgs.taskId) {
                ...
                if (functionArgs.dueDate) {
                  // Обрабатываем инференцию даты
                  let targetDate = functionArgs.dueDate;
                  
                  // Если передали "завтра" или похожее, вычисляем дату
                  if (!functionArgs.dueDate.match(/^\d{4}-\d{2}-\d{2}$/)) {
                    targetDate = formatMoscowDate(1); // завтра
                  }
                  
                  deleteQuery = deleteQuery.eq('due_date', targetDate);
                }

                const { data: foundTasks } = await deleteQuery.limit(50); // Увеличиваем лимит для удаления по дате
