...
export async function updateInternalLinks(
  ...
  const { error } = await supabase.from('internal_link_graph')
    .upsert(inserts, { 
...
export async function suggestInternalLinks(
  ...
  let query = supabase.from('content_ideas')
...
export async function findHubPages(branch?: string): Promise<string[]> {
  ...
  return data?.map((p) => p.route as string) || [];
}
...
export async function getIncomingLinks(
  ...
  const { data, error } = await supabase.from('internal_link_graph')
...
export async function recalculateLinkStrength() {
  // Получаем все ссылки
  const { data: links } = await supabase.from('internal_link_graph')
  ...
  for (const link of links) {
    ...
    await supabase.from('internal_link_graph')
...
export async function findOrphanPages(): Promise<string[]> {
  ...
  const { data: allPages } = await supabase.from('content_ideas')
    .select('route')
    .eq('status', 'published');
  ...
  const { data: linkedPages } = await supabase.from('internal_link_graph')
    .select('to_route');
