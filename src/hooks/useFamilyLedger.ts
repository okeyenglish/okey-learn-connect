...
export const useFamilyLedger = (familyGroupId?: string, clientId?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []) as any as FamilyLedger | null;
    },
...
export const useFamilyLedgerTransactions = (ledgerId?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return (data || []) as any as FamilyLedgerTransaction[];
    },
    enabled: !!ledgerId,
  });
};
...
export const useAddFamilyLedgerTransaction = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data, error } = await supabase.rpc('add_family_ledger_transaction' as any, {
...
export const useTransferToStudentBalance = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data, error } = await supabase.rpc('transfer_to_student_balance' as any, {
